...
public:
  ...
  virtual int call(int64_t m, int64_t n, T* A, int64_t lda, T* R, int64_t ldr,
                   int64_t* J, T d_factor) = 0;
};
...
public:
  ...
  int call(int64_t m, int64_t n, T* A, int64_t lda, T* R, int64_t ldr,
           int64_t* J, T d_factor) override;
...
// -----------------------------------------------------------------------------
template <typename T>
int CQRRPT<T>::call(int64_t m, int64_t n, T* A, int64_t lda, T* R, int64_t ldr,
                    int64_t* J, T d_factor) {
  int i;
  int64_t k = n;
  int64_t d = d_factor * n;
  // A constant for initial rank estimation.
  T eps_initial_rank_estimation =
      2 * std::pow(std::numeric_limits<T>::epsilon(), 0.95);
  ...
  */
  ...
  // auto&& [values, rowptr, colind, shape, _] = spblas::generate_coo<T>(d, m,
  // nnz);
  /// TODO: add COO view
  ...
  /// Using naive rank estimation to ensure that R used for preconditioning is
  /// invertible. The actual rank estimate k will be computed a posteriori.
  /// Using R[i,i] to approximate the i-th singular value of A_hat.
  /// Truncate at the largest i where R[i,i] / R[0,0] >= eps.
  for (i = 0; i < n; ++i) {
    if (std::abs(A_hat[i * d + i]) / std::abs(A_hat[0]) <
        eps_initial_rank_estimation) {
      k = i;
      break;
    }
  }
  ...
  /// Extracting a k by n R representation (k by k upper-triangular, rest -
  /// general)
  lapack::lacpy(MatrixType::Upper, k, k, A_hat, d, R, ldr);
  lapack::lacpy(MatrixType::General, k, n - k, &A_hat[d * k], d, &R[n * k],
                ldr);
  ...
  blas::trsm(Layout::ColMajor, Side::Right, Uplo::Upper, Op::NoTrans,
             Diag::NonUnit, m, k, 1.0, R_sp, k, A, lda);

  // Do Cholesky QR
  blas::syrk(Layout::ColMajor, Uplo::Upper, Op::Trans, k, m, 1.0, A, lda, 0.0,
             R_sp, k);
  ...
  // This also automatically takes care of any potentical failures in Cholesky
  // factorization. Note that the diagonal of R_sp may not be sorted, so we need
  // to keep the running max/min We expect the loss in the orthogonality of Q to
  // be approximately equal to u * cond(R_sp)^2, where u is the unit roundoff
  // for the numerical type T.
  new_rank = k;
  running_max = R_sp[0];
  running_min = R_sp[0];
  ...
  for (i = 0; i < k; ++i) {
    ...
    if (running_max / running_min >=
        std::sqrt(this->eps / std::numeric_limits<T>::epsilon())) {
      new_rank = i - 1;
      break;
    }
  }
  ...
  blas::trsm(Layout::ColMajor, Side::Right, Uplo::Upper, Op::NoTrans,
             Diag::NonUnit, m, new_rank, 1.0, R_sp, k, A, lda);
  // Get the final R-factor.
  blas::trmm(Layout::ColMajor, Side::Left, Uplo::Upper, Op::NoTrans,
             Diag::NonUnit, new_rank, n, 1.0, R_sp, k, R, ldr);
