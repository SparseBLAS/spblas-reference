...
class spgemm {
public:
  spgemm(std::shared_ptr<allocator> memctrl) : memctrl_(memctrl) {
    cusparseCreate(&handle);
    cusparseSpGEMM_createDescr(&spgemmDesc);
  }
  ...
  ~spgemm() {
    cudaDeviceSynchronize();
    cusparseSpGEMM_destroyDescr(spgemmDesc);
    memctrl_->free(dBuffer1);
    memctrl_->free(dBuffer2);
    cusparseDestroySpMat(matA);
    cusparseDestroySpMat(matB);
    cusparseDestroy(handle);
  }
  ...
  operation_info_t multiply_inspect(A&& a, B&& b, C&& c) {
    ...
    cusparseSpGEMM_workEstimation(
        handle, CUSPARSE_OPERATION_NON_TRANSPOSE,
        CUSPARSE_OPERATION_NON_TRANSPOSE, &alpha, matA, matB, &beta, matC,
        cuda_data_type<value_type>(), CUSPARSE_SPGEMM_DEFAULT, spgemmDesc,
        &bufferSize1, NULL);
    memctrl_->alloc(&dBuffer1, bufferSize1);
    // inspect the matrices A and B to understand the memory requirement for
    // the next step
    cusparseSpGEMM_workEstimation(
        handle, CUSPARSE_OPERATION_NON_TRANSPOSE,
    ...
    cusparseSpGEMM_compute(handle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                           ...
                           CUSPARSE_SPGEMM_DEFAULT, spgemmDesc, &bufferSize2,
                           NULL);
    memctrl_->alloc(&dBuffer2, bufferSize2);
    ...
    //  new operation_info_t for nvidia
    auto info = operation_info_t{__backend::shape(c), C_nnz1};
    // we will recureate a new matC in the execution step
    cusparseDestroySpMat(matC);
    return info;
  }
...
private:
  std::shared_ptr<allocator> memctrl_;
  cusparseHandle_t handle = NULL;
