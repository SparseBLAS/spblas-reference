...
auto get_ultimate_base_or_matrix(T&& t) {
  if constexpr (matrix_view::is_general_v<T>) {
    return t;
  } else if constexpr (has_base<T>) {
...
template <tensor T>
bool has_general(T&& t) {
  if constexpr (matrix_view::is_general_v<T>) {
    return true;
  } else if constexpr (has_base<T>) {
    return has_general(t.base());
  } else {
    return false;
  }
}
...
template <typename T>
concept has_general_d = matrix_view::is_general_v<ultimate_base_or_matrix_type_t<T>>;
