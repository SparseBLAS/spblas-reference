...
/*
  ...
  1) A view.  This is a non-owning, lightweight object that reference data owned
     by the user in a particular format.  Since it only views data, but does not own it, it cannot
     reallocate its memory to insert more values if it runs out of space.
     See csr_view below.

  2) An opaque matrix object.  This object owns its data and is able to reallocate
     memory to grow.  When it is constructed, it either creates a copy of user
     data or "steals" it via move construction.  The internal format is opaque
     and unknown to the user.  Extracting data will likely require a copy.
     See matrix below.

  3) A known-format matrix object.  This object owns its data and is able to reallocate
     memory to grow.  When it is constructed, it either creates a copy of user
     data or "steals" it via move construction.  Since its format is known,
     users can easily "steal" its data via a move-like primitive.
*/

template <typename T, typename I = std::size_t, typename O = /* implementation-defined */>
class csr_view {
public:
  ...
  csr_view(I m, I n, O nnz, T* values, I* rowptr, I* colind)
    : m_(m), n_(n), nnz_(nnz), values_(values), rowptr_(rowptr), colind_(colind) {}
