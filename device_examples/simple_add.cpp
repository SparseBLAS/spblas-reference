...
int main(int argc, char** argv) {
  ...
  cudaMalloc((void**) &db_colind, sizeof(int) * b_nnz);
  cudaMemcpy(db_values, b_values.data(), sizeof(float) * b_nnz,
             cudaMemcpyHostToDevice);
  cudaMemcpy(db_rowptr, b_rowptr.data(), sizeof(int) * (shape[0] + 1),
             cudaMemcpyHostToDevice);
  cudaMemcpy(db_colind, b_colind.data(), sizeof(int) * b_nnz,
             cudaMemcpyHostToDevice);
  csr_view<float, int> b(db_values, db_rowptr, db_colind, shape, b_nnz);

  int* dc_rowptr;
  cudaMalloc((void**) &dc_rowptr, sizeof(int) * (shape[0] + 1));
  ...
  float* dc_values;
  int* dc_colind;
  cudaMalloc((void**) &dc_values, handle.result_nnz() * sizeof(float));
  cudaMalloc((void**) &dc_colind, handle.result_nnz() * sizeof(int));
  ...
  cudaMemcpy(c_rowptr.data(), dc_rowptr, sizeof(int) * (shape[0] + 1),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(c_colind.data(), dc_colind, sizeof(int) * handle.result_nnz(),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(c_values.data(), dc_values, sizeof(float) * handle.result_nnz(),
             cudaMemcpyDeviceToHost);
