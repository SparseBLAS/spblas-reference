...
TEST(MixedViews, SpGEMM_CsrCscCsr) {
  using T = float;
...
TEST(MixedViews, SpGEMM_CscCsrCsr) {
  using T = float;
  using I = spblas::index_t;
...
TEST(MixedViews, SpGEMM_CsrCscCsc) {
  using T = float;
  using I = spblas::index_t;
  ...
  for (auto&& [m, k, nnz] : util::dims) {
    for (auto&& n : {m, k}) {
      ...
      std::vector<I> c_ref_colptr(n + 1);

      spblas::csc_view<T, I> c_ref(nullptr, c_ref_colptr.data(), nullptr,
                                   {m, n}, 0);

      info = spblas::multiply_compute(a, b, c_ref);

      std::vector<T> c_ref_values(info.result_nnz());
      std::vector<I> c_ref_rowind(info.result_nnz());

      c_ref.update(c_ref_values, c_ref_colptr, c_ref_rowind);

      spblas::multiply_fill(info, a, b, c_ref);

      spblas::__backend::spa_accumulator<T, I> c_column_acc(c.shape()[0]);

      for (auto&& [j, c_column] : spblas::__backend::columns(c)) {
        c_column_acc.clear();

        auto&& c_ref_column = spblas::__backend::lookup_column(c_ref, j);

        EXPECT_EQ(c_column.size(), c_ref_column.size());

        for (auto&& [i, v] : c_column) {
          c_column_acc[i] += v;
        }

        for (auto&& [i, v] : c_ref_column) {
          EXPECT_EQ_(v, c_column_acc[i]);
        }
      }
    }
  }
...
TEST(MixedViews, SpGEMM_CscCsrCsc) {
  using T = float;
  using I = spblas::index_t;
