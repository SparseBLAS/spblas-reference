...
TEST(thrust_CsrView, SpGEMM_4Args) {
  for (auto&& [m, k, nnz] : util::dims) {
    for (auto&& n : {m, k}) {
      ...
      auto [d_values, d_rowptr, d_colind, d_shape, d_nnz] =
          spblas::generate_csr<value_t, index_t, offset_t>(m, n, nnz);
      thrust::device_vector<value_t> d_d_values(d_values);
      thrust::device_vector<offset_t> d_d_rowptr(d_rowptr);
      thrust::device_vector<index_t> d_d_colind(d_colind);
      spblas::csr_view<value_t, index_t, offset_t> d_d(
          d_d_values.data().get(), d_d_rowptr.data().get(),
          d_d_colind.data().get(), d_shape, d_nnz);
      spblas::csr_view<value_t, index_t, offset_t> d(d_values, d_rowptr,
                                                     d_colind, d_shape, d_nnz);
      ...
      spblas::multiply_compute(state, d_a, d_b, d_c, d_d);
      ...
      spblas::multiply_fill(state, d_a, d_b, d_c, d_d);
      ...
      for (auto&& [i, a_row] : spblas::__backend::rows(a)) {
        ...
        auto&& d_row = spblas::__backend::lookup_row(d, i);
        for (auto&& [k, d_v] : d_row) {
          c_row_ref[k] += d_v;
        }
...
TEST(thrust_CsrView, SpGEMM_4Args_AScaled) {
  value_t alpha = 2.0f;
  for (auto&& [m, k, nnz] : util::dims) {
    for (auto&& n : {m, k}) {
      auto [a_values, a_rowptr, a_colind, a_shape, a_nnz] =
      ...
      spblas::spgemm_state_t state;
      spblas::multiply_compute(state, scaled(alpha, d_a), d_b, d_c, d_d);
      auto nnz = state.result_nnz();
      thrust::device_vector<value_t> d_c_values(nnz);
      thrust::device_vector<index_t> d_c_colind(nnz);
      std::span<value_t> d_c_values_span(d_c_values.data().get(), nnz);
      ...
      spblas::multiply_fill(state, scaled(alpha, d_a), d_b, d_c, d_d);
...
TEST(thrust_CsrView, SpGEMM_4Args_BScaled) {
  value_t alpha = 2.0f;
  for (auto&& [m, k, nnz] : util::dims) {
    for (auto&& n : {m, k}) {
      auto [a_values, a_rowptr, a_colind, a_shape, a_nnz] =
      ...
      spblas::spgemm_state_t state;
      spblas::multiply_compute(state, d_a, scaled(alpha, d_b), d_c, d_d);
      auto nnz = state.result_nnz();
      thrust::device_vector<value_t> d_c_values(nnz);
      thrust::device_vector<index_t> d_c_colind(nnz);
      std::span<value_t> d_c_values_span(d_c_values.data().get(), nnz);
      ...
      spblas::multiply_fill(state, d_a, scaled(alpha, d_b), d_c, d_d);
...
TEST(thrust_CsrView, SpGEMM_4Args_DScaled) {
  value_t alpha = 2.0f;
  for (auto&& [m, k, nnz] : util::dims) {
    for (auto&& n : {m, k}) {
      auto [a_values, a_rowptr, a_colind, a_shape, a_nnz] =
      ...
      spblas::spgemm_state_t state;
      spblas::multiply_compute(state, d_a, d_b, d_c, scaled(alpha, d_d));
      auto nnz = state.result_nnz();
      thrust::device_vector<value_t> d_c_values(nnz);
      thrust::device_vector<index_t> d_c_colind(nnz);
      std::span<value_t> d_c_values_span(d_c_values.data().get(), nnz);
      ...
      spblas::multiply_fill(state, d_a, d_b, d_c, scaled(alpha, d_d));
